...
from info import AUTH_CHANNEL, LONG_IMDB_DESCRIPTION, MAX_LIST_ELM, LOG_CHANNEL, GRP_LNK, CHNL_LNK, CUSTOM_FILE_CAPTION, IS_VERIFY, VERIFY2_URL, VERIFY2_API, PROTECT_CONTENT, HOW_TO_VERIFY, JK_DEV_API, JK_DEV_URL
...
async def import_jk_dev(url, api, link):
...
async def send_all(bot, userid, files, ident):
    if AUTH_CHANNEL and not await is_subscribed(bot=bot, userid=userid):
        ...
        if ident == 'filep' or 'checksubp':
            ...
            pre = 'checksub' 
        btn = [[
                InlineKeyboardButton("❆ Jᴏɪɴ Oᴜʀ Bᴀᴄᴋ-Uᴘ Cʜᴀɴɴᴇʟ ❆", url=AUTH_CHANNEL)
            ],[
        ...
        try:
            await bot.send_cached_media(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                        InlineKeyboardButton("🖥️ Wᴀᴛᴄʜ & Dᴏᴡɴʟᴏᴀᴅ 📥", callback_data=f"jk_streaming#{file.file_id}") 
                        ],[
